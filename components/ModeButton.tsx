...
export function ModeButton({
  ...
  const combinedClassName = `inline-flex min-h-11 items-center justify-center border-2 border-black px-4 py-2 text-center text-xs sm:text-sm font-black uppercase rounded-xl shadow-[3px_3px_0_#111111] cursor-pointer transition-all duration-150 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[4.5px_4.5px_0_#111111] active:translate-x-0.5 active:translate-y-0.5 active:shadow-[1.5px_1.5px_0_#111111] ${variantClasses[variant]} ${
    disabled ? "pointer-events-none opacity-50 shadow-none border-zinc-300" : ""
  } ${className}`;

  if (href) {
    return (
      <Link className={combinedClassName} href={href} aria-disabled={disabled}>
        {children}
      </Link>
    );
  }
  ...
  return (
    <button className={combinedClassName} onClick={onClick} type="button" disabled={disabled}>
      {children}
    </button>
  );
}
